...
`launch` builds the app, checks xAI OAuth, opens the browser login flow when no token exists, then starts the web app. Open `http://127.0.0.1:8787` and enter `ACCESS_TOKEN` from `.env`.

## Configuration

- `ACCESS_TOKEN` gates API access. Keep it set even for local-only use.
