...
export async function resolveXaiAuth(config: AppConfig): Promise<XaiRuntimeAuth> {
  ...
  if (accessToken && !accessTokenIsExpiring(accessToken, state.tokens?.expires_at)) {
    return {
      token: accessToken,
      label: "xai-oauth",
      baseUrl: cleanBaseUrl(state.base_url ?? config.xai.baseUrl),
    };
  }
  ...
  return {
    ...
    label: "xai-oauth",
    baseUrl: cleanBaseUrl(refreshed.base_url ?? config.xai.baseUrl),
  };
}
...
async function refreshTokenState(state: XaiOAuthTokenState): Promise<XaiOAuthTokenState> {
  ...
  const discovery = state.discovery?.token_endpoint
    ? (state.discovery as XaiOAuthDiscovery)
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `xAI OAuth refresh failed: ${response.status} ${response.statusText} - ${body.slice(0, 400)}`,
    );
  }
  const payload = (await response.json()) as Record<string, unknown>;
  const accessToken = cleanBearerToken(stringValue(payload.access_token));
  ...
  return {
    ...
    tokens: {
      ...
      refresh_token: cleanBearerToken(stringValue(payload.refresh_token)) ?? refreshToken,
...
async function fetchDiscovery(): Promise<XaiOAuthDiscovery> {
  ...
  if (!response.ok)
    throw new Error(`xAI OIDC discovery failed: ${response.status} ${response.statusText}`);
...
function jwtExpiryMs(token: string): number | undefined {
  ...
  try {
    ...
    const json = JSON.parse(Buffer.from(normalized, "base64url").toString("utf8")) as {
      exp?: unknown;
    };
