...
export const ASPECT_RATIOS = ["source", "auto", "1:1", "16:9", "9:16", "4:3", "3:4", "3:2", "2:3"] as const;
export type AspectRatioOption = typeof ASPECT_RATIOS[number];

export const CAMERA_MODES = ["static", "user", "push_in", "pull_back", "orbit", "handheld"] as const;
...
export const VIDEO_MOTION_PRESETS: readonly VideoMotionPreset[] = [
  ...
  },
  {
    ...
    prompt: "subject slowly turns their head for a brief over-the-shoulder glance, then settles naturally",
    category: "portrait",
    durationSeconds: 6,
  },
  {
  ...
  },
  {
    ...
    prompt: "subject turns slowly from front view to a three-quarter side angle and back, elegant controlled outfit presentation",
    category: "showcase",
    durationSeconds: 8,
    camera: "static",
  },
  ...
  },
  {
    ...
    prompt: "a controlled short dolly orbit with subtle parallax, stable framing, no aggressive spin",
    category: "cinematic",
    durationSeconds: 8,
    camera: "orbit",
  },
  {
    ...
    prompt: "slow controlled product-style orbit, highlight materials and surface details, premium studio motion",
    category: "product",
    durationSeconds: 8,
    camera: "orbit",
  },
...
export function normalizeGenerationOptions(input: unknown, fallback: GenerationOptions, maxVariations: number): GenerationOptions {
  ...
  if (preset?.durationSeconds && !parsed.durationSeconds) merged.durationSeconds = preset.durationSeconds;
  if (preset?.camera && !parsed.camera) merged.camera = preset.camera;
  return generationOptionsSchema.parse(merged);
}

export function composePrompt(options: Pick<GenerationOptions,
  "prompt" | "presetId" | "camera" | "sound" | "intensity" | "outputStyle" | "preserveSource" | "avoidTextMutation" | "loopFriendly"
>): string {
  ...
  if (options.preserveSource) {
    parts.push("Preserve the source image identity, composition, subject position, outfit, proportions, and visual style.");
    parts.push("Keep motion natural and controlled; avoid deformation by keeping the subject stable and readable.");
  }
  if (options.avoidTextMutation) {
    parts.push("Do not invent or rewrite readable text, logos, watermarks, tattoos, UI text, or signage.");
  }
  ...
  if (options.loopFriendly) parts.push("Make the first and last frames visually compatible for a clean loop.");
...
function styleDirective(style: OutputStyle): string {
  ...
  if (style === "social") return "Clean social-media-ready motion, crisp subject readability, no clutter.";
  if (style === "product") return "Premium product-shot finish with controlled highlights and readable materials.";
...
function intensityDirective(intensity: MotionIntensity): string {
  ...
  if (intensity === "bold") return "Use a clear motion beat while keeping anatomy, identity, and composition stable.";
...
function cameraDirective(mode: CameraMode, prompt: string): string {
  ...
  if (mode === "orbit") return "Use a short controlled orbit with subtle parallax, no spin-heavy movement.";
  if (mode === "handheld") return "Use gentle handheld camera feel with stable framing, no shake-heavy movement.";
  if (/\b(camera|cam|镜头|pan|tilt|zoom|push|pull|dolly|orbit|handheld|static)\b|推近|拉远|环绕|跟拍|平移|手持/i.test(prompt)) {
    return "";
  }
...
function soundDirective(mode: SoundMode): string {
  ...
  if (mode === "music") return "Use quiet background music and ambient sound only; no dialogue or lip sync.";
