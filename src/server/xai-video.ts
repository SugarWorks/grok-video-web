...
export async function generateGrokImageVideo(input: GenerateVideoInput): Promise<VideoJobResult> {
  if (!fs.existsSync(input.imagePath)) throw new Error(`source image does not exist: ${input.imagePath}`);
  ...
  const submit = await fetchWithRetry(`${auth.baseUrl}/videos/generations`, {
    method: "POST",
    headers: {
  ...
  if (!submit.ok) {
    ...
    throw new Error(`Grok submit failed: ${submit.status} ${submit.statusText} - ${body.slice(0, 500)}`);
  }
  const submitJson = await submit.json() as { request_id?: string };
  const requestId = submitJson.request_id;
  if (!requestId) throw new Error(`Grok submit returned no request_id: ${JSON.stringify(submitJson).slice(0, 300)}`);
...
export function buildXaiVideoRequest(config: AppConfig, imagePath: string, options: GenerationOptions): Record<string, unknown> {
...
async function pollUntilDone(input: {
  ...
  while (Date.now() < deadline) {
    ...
    const response = await fetchWithRetry(`${input.baseUrl}/videos/${input.requestId}`, {
      headers: { "Authorization": `Bearer ${input.token}` },
    }, "poll");
    if (!response.ok) {
      const body = await response.text().catch(() => "");
      throw new Error(`Grok poll failed: ${response.status} ${response.statusText} - ${body.slice(0, 500)}`);
    }
    ...
    if (status === "done") {
      ...
      if (!url) throw new Error(`Grok poll returned done without video URL: ${JSON.stringify(json).slice(0, 300)}`);
      return url;
    }
  ...
  throw new Error(`Grok video timed out after ${input.timeoutSeconds}s for request ${input.requestId}`);
}

async function downloadVideo(url: string, workspaceDir: string, jobId: string, requestId: string): Promise<string> {
  const response = await fetchWithRetry(url, {}, "download");
  if (!response.ok) throw new Error(`Grok download failed: ${response.status} ${response.statusText}`);
  const outputDir = path.join(workspaceDir, "videos");
  fs.mkdirSync(outputDir, { recursive: true });
  const filePath = path.join(outputDir, `${new Date().toISOString().replace(/[:.]/g, "-")}-${jobId}-${requestId.slice(0, 8)}.mp4`);
  fs.writeFileSync(filePath, Buffer.from(await response.arrayBuffer()));
  return filePath;
}

async function fetchWithRetry(url: string, init: RequestInit, purpose: "submit" | "poll" | "download"): Promise<Response> {
  ...
  for (let attempt = 0; attempt < delays.length; attempt += 1) {
    ...
    try {
      ...
      lastError = error;
      console.warn(`[grok-video-fetch] purpose=${purpose} attempt=${attempt + 1}/${delays.length} error=${formatError(error)}`);
    }
  }
